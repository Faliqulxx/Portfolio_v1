...
const Lanyard = dynamic(() => import("@/components/Lanyard"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full min-h-[300px] flex items-center justify-center text-xs text-gray-400 dark:text-white/40">
...
export default function AIAssistantPage() {
  ...
  return (
    <div className="h-screen max-h-screen pt-20 sm:pt-24 pb-4 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col items-center overflow-hidden box-border">
      ...
      <div className="w-full flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-stretch">
        {/* LEFT COLUMN: CHAT INTERFACE */}
        <motion.div
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.4, delay: 0.1 }}
          className="lg:col-span-7 xl:col-span-7 flex flex-col rounded-2xl sm:rounded-3xl border border-gray-200 dark:border-white/10 bg-white/70 dark:bg-gray-900/60 backdrop-blur-xl shadow-2xl overflow-hidden h-full min-h-0"
        >
          {/* TOP BAR */}
          ...
          >
            {isEmpty ? (
              ...
              >
                <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 flex items-center justify-center text-indigo-600 dark:text-indigo-400 text-2xl mb-2 shadow-inner">
                  <HiOutlineLightBulb />
                </div>
                <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-1">
                  Halo! Ada yang bisa saya bantu hari ini?
                </h3>
                <p className="text-xs text-gray-500 dark:text-white/50 max-w-md mb-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full max-w-2xl">
                  {QUICK_PROMPTS.map((item, idx) => (
                    <motion.button
                      ...
                      className="p-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white/60 dark:bg-white/5 hover:bg-indigo-50/50 dark:hover:bg-indigo-500/10 hover:border-indigo-300 dark:hover:border-indigo-500/30 text-left transition-all group shadow-sm flex flex-col justify-between"
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-gray-500 dark:text-white/50 line-clamp-2 leading-snug">
                          "{item.description}"
                        </p>
                      </div>
                    </motion.button>
        ...
        {/* RIGHT COLUMN: 3D INTERACTIVE LANYARD CARD */}
        <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.4, delay: 0.2 }}
          className="hidden lg:flex lg:col-span-5 xl:col-span-5 flex-col items-center justify-center rounded-2xl sm:rounded-3xl border border-gray-200 dark:border-white/10 bg-white/40 dark:bg-gray-900/40 backdrop-blur-xl shadow-xl overflow-hidden h-full min-h-0 relative group"
        >
          {/* Card Label */}
          <div className="absolute top-4 left-4 z-10 px-3 py-1 rounded-full bg-white/70 dark:bg-gray-950/70 border border-gray-200 dark:border-white/10 text-[11px] font-semibold text-gray-700 dark:text-white/80 backdrop-blur-md shadow-sm">
            🪪 Interactive ID Card
          </div>
          <p className="absolute bottom-4 z-10 text-[11px] text-gray-500 dark:text-white/40 bg-white/60 dark:bg-gray-950/60 px-3 py-1 rounded-full border border-gray-200 dark:border-white/10 backdrop-blur-md">
            Click & drag card to physics interact
          </p>
